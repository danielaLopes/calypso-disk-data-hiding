/*
 * sbull.c - Sample disk driver, from the beginning.
 * 
 * Source code in: https://github.com/martinezjavier/ldd3/blob/master/sbull/sbull.c
 */

/*
 * Experiments: each read to sbulla (or other device) issues 
 * requests from sector 0 to 1023, with step 8
 * 
 * We have 1024 sectors, each with 512 bytes
 * So it makes sense we have a read going from sector 
 * 0 to sector 1023
 */
...
static int nsectors = 1024;	/* How big each drive is */
 ...
 * In this simple mode of operation, requests are taken off the 
 * queue only when they are complete.
 */
 ...
 * This is a good example to use when we need to dig 
 * through the bio structures
 */
 ...
 * Better performance for RAM only devices and it does not 
 * use a request queue
 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 9, 0))
...
static void setup_device(struct sbull_dev *dev, int which)
{
	...
	/*
	 * add_disk issues around 3 requests per device
	 * for instance sec 0, 8 and 24
	 */
	add_disk(dev->gd);
	printk (KERN_INFO "after add_disk()\n");
	return;
